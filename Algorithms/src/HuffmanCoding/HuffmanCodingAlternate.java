...
public class HuffmanCodingAlternate {
	...
	public static MinHpNode buildHuffmanTree(char data[], int freq[], int size) {
		
		MinHpNode left, right, top;
		
		// Step 1 : Create a min heap of capacity equal to the size. 
		PriorityQueue<MinHpNode> minHeap = new PriorityQueue<MinHpNode>(size);
		
		// Step 2 : Build the heap
		for (int i=0; i<data.length; i++) {
			minHeap.add(new MinHpNode(data[i], freq[i]));
		}
		...
		while (minHeap.size() != 1) {
			...
			top = new MinHpNode('$', left.freq + right.freq);
			top.left = left;
			top.right = right;
			minHeap.add(top);
		}
	...
	public static void printCodes(MinHpNode root, int arr[], int top) {
	...
	public static void HuffmanCodes(char data[], int freq[], int size) {
		...
		MinHpNode root = buildHuffmanTree(data, freq, size);
...
class MinHpNode implements Comparable<MinHpNode> {
	
	public char data;
	public int freq;
	MinHpNode left;
	public MinHpNode right;
	
	public MinHpNode() {}
	
	public MinHpNode(char data, int freq) {
		this.data = data;
		this.freq = freq;
	}
	
	
	public int compareTo(MinHpNode o) {
